...
test('renamed generic replacement updates receiver owner', () => {
  const output = transformReplacement(
    `declare abstract class Owner {
      method(this: __JSG_GENERATED_RECEIVER__<Owner>): void;
    }`,
    `interface RenamedOwner<U> {
      method(): void;
    }`
  );
  assert.match(output, /interface RenamedOwner<U>/);
  assert.match(
    output,
    /method\(this: __JSG_GENERATED_RECEIVER__<RenamedOwner<U>>\): void;/
  );
  assert.doesNotMatch(output, /__JSG_GENERATED_RECEIVER__<Owner>/);
});
