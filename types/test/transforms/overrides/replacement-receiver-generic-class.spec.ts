...
test('generic generated owner to nongeneric class replacement', () => {
  ...
  assert.match(output, /declare abstract class Owner/);
  assert.match(
    output,
    /method\(this: __JSG_GENERATED_RECEIVER__<Owner>\): void;/
  );
...
test('generic generated owner to generic class replacement', () => {
  const output = transformReplacement(
    `declare abstract class Owner<T> {
      method(this: __JSG_GENERATED_RECEIVER__<Owner>): void;
    }`,
    `declare abstract class Owner<U> {
      method(): void;
    }`
  );
  assert.match(output, /declare abstract class Owner<U>/);
  assert.match(
    output,
...
test('nongeneric generated owner to generic class replacement', () => {
  const output = transformReplacement(
    `declare abstract class Owner {
      method(this: __JSG_GENERATED_RECEIVER__<Owner>): void;
    }`,
