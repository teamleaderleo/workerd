...
// Assigning to a receiver-free callback type intentionally erases the explicit
// `this` parameter, matching TypeScript's normal function assignability.
const erasedReceiver: (
  ...args: Parameters<typeof fetch>
) => ReturnType<typeof fetch> = fetch;
erasedReceiver(url);
({ fetch: erasedReceiver }).fetch(url);
